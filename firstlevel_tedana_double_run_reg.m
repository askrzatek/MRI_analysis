...
model_name = {'full_sts_tapas_doublerun_resliced_emg'};%, 'smodel_ts_tapas', 'smodel_dn_tapas'};
...
for ip = 1 : length(patient_regex)
    ...
    emg = exam(stim_dir,patient_regex{ip});
    reg_regex = {'.*EXT_D__reg.mat','.*EXT_G__reg.mat','.*FLE_D__reg.mat','.*FLE_G__reg.mat'};
    %emg_reg = cellstr(get_subdir_regex_files(emg.gpath,'EXT_D__reg.mat',4))
    
    if length(esuj) == 2
       dirFonc(ip,:) = esuj.getSerie('run_ACTIVATION') .toJob;
       stim_files(ip,:) = esuj.getStim.toJob(0);
       for im = 1 : 4
         emg_reg(ip,1,im) = get_subdir_regex_files(emg(1).gpath,reg_regex{im},1);
         emg_reg(ip,2,im) = get_subdir_regex_files(emg(2).gpath,reg_regex{im});
       end
    end
end
...
job_doublerun_model_specify_reg(dirFonc, patients_dir, stim_files, emg_reg, par)
