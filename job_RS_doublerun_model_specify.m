...
%%
n = 1;
for ip = 1 : length(dirFonc{1,:,:})
    for ir = 1 : length(dirFonc{:,})
        jobs{n}.spm.stats.fmri_spec.dir = dirOutput{n};
        ...
        if par.rp
            jobs{n}.spm.stats.fmri_spec.sess(1).multi_reg = get_subdir_regex_files(dirFonc{i,1},par.rp_regex);
        end
        ...
        if par.rp
            jobs{n}.spm.stats.fmri_spec.sess(2).multi_reg = get_subdir_regex_files(dirFonc{i,2},par.rp_regex);
        end
        ...
        if par.mask
            if par.mask_path{1} == ''
                jobs{n}.spm.stats.fmri_spec.mask = unzip_volume(get_subdir_regex_files(dirFonc{i,2},'wbet.*mask'));
            else
