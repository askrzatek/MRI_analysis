function RS_first_level_batch_preprocessing (par)
    clear all
    clc
...
%% defpar
    defpar.anat_file_reg = '^s.*p2.nii';
    defpar.rs_file_reg = '^S.*T1c_medn_nat.nii';
    defpar.sge = 0;
    defpar.redo = 0;
    defpar.fake = 0;
    defpar.verbose = 2;
...
%% CREATE A FOLDER conn_RS_data at the level of all subjects (/network/lustre/iss02/cenir/analyse/irm/users/anna.skrzatek/nifti/)
    ...
    working_dir = char(r_mkdir(main_dir,'conn_RS_data_preproc'));
...
%% Session 1
    ...
    e_REMINARY_S1.getSerie('run_RS').addVolume(par.rs_file_reg,'wT1c',1);
    e_REMINARY_S1.addSerie('t1mpr_S256_0_8iso_p2$','anat_T1',1)
    e_REMINARY_S1.getSerie('anat_T1').addVolume(par.anat_file_reg,'s',1)
...
%% Session 2
    ...
    e_REMINARY_S2.getSerie('run_RS').addVolume(par.rs_file_reg,'wT1c',1);
    e_REMINARY_S2.addSerie('t1mpr_S256_0_8iso_p2$','anat_T1',1)
    e_REMINARY_S2.getSerie('anat_T1').addVolume(par.anat_file_reg,'s',1)
...
%%
    ...
    for n=1:numel(session_dir),
        ...
        for i=1:numel(subj_dir)
            ...
            tFUNCTIONAL_FILE=cellstr(conn_dir(fullfile(subjpath,'RS',   'S05_RS_T1c_medn_nat.nii')));
            tSTRUCTURAL_FILE=cellstr(conn_dir(fullfile(subjpath,'anat', 's_S03_t1mpr_S256_0_8iso_p2.nii')));
    ...
    clear batch;
    batch.filename=fullfile(cwd,'conn_RS_REMINARY_preproc.mat');            % New conn_*.mat experiment name
...
%
    ...
    conn
    conn('load',fullfile(cwd,'conn_RS_REMINARY_preproc.mat'));
    conn gui_results
end
