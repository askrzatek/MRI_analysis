function matlabbatch = secondlevel_RS_2s_matlabbatch(groups,outdirs,covars,par)
%%
...
%%
ijob = 1;
for icon = 1 : length(groups{1})
    
    jobs{ijob}.spm.stats.factorial_design.dir = outdirs{1}(icon);
    jobs{ijob}.spm.stats.factorial_design.des.t2.scans1 = spm_select('expand',groups{1}{icon});
    jobs{ijob}.spm.stats.factorial_design.des.t2.scans2 = spm_select('expand',groups{2}{icon});
